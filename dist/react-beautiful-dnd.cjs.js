...
var getReorderImpact = (function (_ref2) {
  ...
  var uprightAxis = axis.direction === 'horizontal' ? vertical : horizontal;
  var uprightAxisBoundStart = targetRect[uprightAxis.start];
  var withoutDragging = removeDraggableFromList(draggable, insideDestination);
  var closest = find(withoutDragging, function (child) {
    ...
    if (axis.grid) {
      var uprightChildCenter = child.page.borderBox.center[uprightAxis.line];
...
var getDroppableDimension = (function (_ref) {
  ...
  var axis = direction === 'vertical' ? vertical : direction === 'grid' ? grid : horizontal;
  var subject = getSubject({
    page: page,
    withPlaceholder: null,
    axis: axis,
...
function getSecondarySelector() {
  ...
  var getProps = function getProps(ownId, draggingId, impact, afterCritical, overrideDisplacement) {
    ...
    var displaceBy = overrideDisplacement || impact.displacedBy.point;
    var offset = memoizedOffset(displaceBy.x, displaceBy.y);
    return getMemoizedProps(offset, combineTargetFor, visualDisplacement.shouldAnimate);
  };

  var getDisplacementOverride = function getDisplacementOverride(state, ownProps) {
    if (!state.impact || !state.onLiftImpact || !state.dimensions) return null;
    var impact = state.onLiftImpact;
    if (impact.displaced.all.length < state.impact.displaced.all.length) impact = state.impact;
    var dims = state.dimensions.draggables;
    var displaced = impact.displaced.all;
    var index = displaced.indexOf(ownProps.draggableId);

    if (index > -1 && displaced.length > index + 1) {
      var ownDimensions = dims[ownProps.draggableId].client.borderBox;

      if (index > 0) {
        var prevDraggableId = displaced[index - 1];
        var prevDraggableDimensions = dims[prevDraggableId].client.borderBox;
      ...
      var nextDraggableId = displaced[index + 1];
      var nextDraggableDimensions = dims[nextDraggableId].client.borderBox;

      if (nextDraggableDimensions.y !== ownDimensions.y) {
        if (!impact.displaced.visible[ownProps.draggableId].shouldAnimate) return {
  ...
  var selector = function selector(state, ownProps) {
    if (state.isDragging) {
      ...
      if (state.critical.droppable && state.dimensions.droppables[state.critical.droppable.id].axis.grid) {
        return getProps(ownProps.draggableId, state.critical.draggable.id, state.impact, state.afterCritical, getDisplacementOverride(state, ownProps));
      }
    ...
    if (state.phase === 'DROP_ANIMATING') {
      ...
      if (state.completed.critical.droppable && state.dimensions.droppables[state.completed.critical.droppable.id].axis.grid) {
        return getProps(ownProps.draggableId, completed.result.draggableId, completed.impact, completed.afterCritical, getDisplacementOverride(state, ownProps));
      }
